...
export abstract class BaseController {
  public static jsonResponse(res: express.Response, code: STATUS_CODE, message?: string) {
    const returnMessage: string = message ? message : MessageStatusMapping[code];
  ...
  public created<T>(res: express.Response, dto?: T): express.Response<T> {
    if (!!dto) {
      res.type('application/json');
      return res.status(STATUS_CODE.CREATED).json(dto);
    } else {
  ...
  public paymentRequired(res: express.Response, message?: string) {
    return BaseController.jsonResponse(res, STATUS_CODE.PAYMENT_REQUIRED, message);
  }
  ...
  public tooMany(res: express.Response, message?: string) {
    return BaseController.jsonResponse(res, STATUS_CODE.TOO_MANY_REQUESTS, message);
  }

  public fail(res: express.Response, error: Error | BaseError) {
    ...
    return res.status(STATUS_CODE.SERVER_INTERNAL_ERROR).json({
      statusCode: STATUS_CODE.SERVER_INTERNAL_ERROR,
      message: error.message,
      errorCode: ERROR_CODE.SERVER_INTERNAL_ERROR,
    });
  }

  public badRequest(res: express.Response, error: Error) {
